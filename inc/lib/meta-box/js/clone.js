jQuery( function ( $ )
{
	...
	$( '#poststuff' ).on( 'click', '.add-clone', function ( e )
	{
		...
		if ( $input.find('.ct_datepicker').length ) {
			$input.find('.ct_datepicker').datepicker('destroy');
			$input.find('.ct_datepicker').removeClass("hasDatepicker").removeAttr('id');
		}
		...
		if ( $input.find('.ct_datepicker').length ) {
			$input.find('.ct_datepicker').datepicker({ dateFormat: "yy-mm-dd" });
		}
	} )
	// Remove clones
	.on( 'click', '.remove-clone', function ( e )
	{
		...
		$this.closest('.rwmb-clone').remove();
